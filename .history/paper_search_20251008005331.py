...
try:
    ...
    # TIP: The Graph API doesn't do Boolean syntax.
    # Keep it simple and run one or more plain-text queries.
    QUERIES = [
        'reinforcement learning "mechanical ventilation" weaning',
        'reinforcement learning "mechanical ventilation" extubation',
        'reinforcement learning "spontaneous breathing trial"',
    ]

    BASE = "https://api.semanticscholar.org/graph/v1/paper/search"
    # Valid fields. 'doi' is not a top-level field; use externalIds.DOI.
    FIELDS = "paperId,title,year,venue,url,externalIds,authors"
    LIMIT = 100        # API allows up to 100
    PAGES = 5          # adjust as needed
    SLEEP_SEC = 0.6    # be gentle with rate limits

    # Optional: set SEMANTIC_SCHOLAR_API_KEY in your env to increase reliability.
    ...
    for query in QUERIES:
        for page in range(PAGES):
            ...
            # If something goes wrong, print the server's message to help debugging
            if r.status_code != 200:
                try:
                    err_body = r.json()
                except Exception:
                    err_body = r.text
                raise RuntimeError(f"S2 error {r.status_code} for query={query}, offset={offset}: {err_body}")

            data = r.json()

            # The search endpoint returns 'total'
            total_reported = data.get("total")
            totals_reported.append({"query": query, "total": total_reported})
    ...
    # Save a single consolidated JSON (not one file per page)
    ...
    print(f"Semantic Scholar combined items: {len(all_items)}")

except Exception as e:
    # Also save the full error text to help diagnose 400s etc.

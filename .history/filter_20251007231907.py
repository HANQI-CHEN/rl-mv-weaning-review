...
def apply_filters(records):
    ...
    for r in records:
        ...
        if has_mv and has_wean and (has_rl or not STRICT_REQUIRE_RL):
            ...
            r["match_rl"] = has_rl
            screened_in = screened_in + [r]
        else:
            reason = []
            if not has_mv: reason.append("no_mv")
            if not has_wean: reason.append("no_weaning")
            if STRICT_REQUIRE_RL and not has_rl: reason.append("no_rl")

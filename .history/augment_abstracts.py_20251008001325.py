...
- Input : paper_list/normalized/excluded_non_mv_weaning.csv
- Output: paper_list/normalized/abstract_check.csv (and .jsonl)
- Also updates paper_list/normalized/prisma_counts.json with abstract_rescreen info.

Logic:
  If abstract contains MV AND Weaning (and RL if STRICT_REQUIRE_RL=True) -> decision="rescue"
  If abstract fetched but no MV+Weaning -> decision="confirmed_exclude"
  If abstract not fetched -> decision="keep_no_abstract" (kept for manual review)
"""
...
# Term lists (same spirit as your filter)
RL_TERMS = [
...
def compile_terms(terms):
    ...
    # keep regex for ones like \bMDP\b, and escape simple words
    rx = []
    for t in terms:
...
def main():
    ...
    # 2) Semantic Scholar as fallback (primarily by DOI, then remaining PMIDs)
    ...
    for r in rows:
        ...
        # match on abstract only (per your request)
    ...
    # save outputs
    save_csv(OUT_CHECK_CSV, out_rows)
    save_jsonl(OUT_CHECK_JL, out_rows)
    print(f"Saved:\n - {OUT_CHECK_CSV}\n - {OUT_CHECK_JL}")

    # update PRISMA counts
    abstract_rescreen = {
        "checked": len(out_rows),
        "rescued": rescued,
        "confirmed_excluded": confirmed,
        "no_abstract": noabs
    }
    # read existing
    prisma = {}

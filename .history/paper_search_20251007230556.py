...
try:
    ...
    FIELDS = "title,year,authors,venue,externalIds,doi,url"  # richer fields for later screening
    LIMIT = 100      # items per page (API allows up to 100)
    PAGES = 5        # how many pages to fetch (adjust as needed)
    SLEEP_SEC = 0.5  # gentle pacing
    ...
    combined = {
        ...
        "items": all_items  # raw items concatenated; no dedup/filtering
    }
    save_json("semanticscholar_all.json", combined)
...
try:
    ...
    all_items = []  # just concatenate items from each page (no processing)
    pages_fetched = 0
    ...
    # save a single combined JSON file with minimal metadata + all items
    combined = {
        "query": q,

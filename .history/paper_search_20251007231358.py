...
try:
    ...
    FIELDS = "title,year,authors,venue,externalIds,doi,url"
    LIMIT = 100      # API allows up to 100
    PAGES = 5        # adjust if needed
    SLEEP_SEC = 0.5
    ...
    for page in range(PAGES):
        ...
        if total_reported is None:
            ...
            print("Semantic Scholar total (reported):", total_reported)

        items = data.get("data", [])
        print(f"Semantic Scholar page {page+1} (offset={offset}) fetched: {len(items)}")
        if not items:
            break
...
try:
    ...
    MAX_RESULTS = 200     # per page (arXiv limit)
    PAGES = 50            # raise/lower if needed
    SLEEP_SEC = 3.0       # arXiv recommends ~3s between calls

    pages_fetched, all_pages = 0, []
    for page in range(PAGES):
        start = page * MAX_RESULTS
        r = requests.get(BASE, params={"search_query": q, "start": start, "max_results": MAX_RESULTS}, timeout=60)
        r.raise_for_status()
        xml_text = r.text
        all_pages.append({"start": start, "xml": xml_text})
        pages_fetched += 1
        # stop early if no entries
        if "<entry>" not in xml_text:
            break
        print(f"arXiv page {page+1} (start={start}) saved")
        time.sleep(SLEEP_SEC)
    ...
    save_json("arxiv_all.json", {
        ...
        "pages_fetched": pages_fetched,
        "pages": all_pages  # store raw Atom XML per page; no parsing now
    })
except Exception as e:
